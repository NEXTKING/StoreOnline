...
{
    NSString *oldPriceParam = item.additionalParameters[@"Старая цена"];
    NSString *firstPriceParam = item.additionalParameters[@"sale"];
    NSString *saleParam = item.additionalParameters[@"FirstPrice"];
    
    if (saleParam && firstPriceParam && oldPriceParam  && [saleParam isEqualToString:@"1"] && (item.price <= oldPriceParam.doubleValue))
    {
        
        NSString *addString = [NSString stringWithFormat:@"%@ %@ %@%@", _dateLabel.text, NSLocalizedString(@"Ст.цена", nil), firstPriceParam, NSLocalizedString(@"р.", nil)];
        NSMutableAttributedString *mutableAttr = [[NSMutableAttributedString alloc] initWithString:addString];
        NSInteger location = [addString rangeOfString:firstPriceParam].location;
