...
- (void) run: (NSString*) url
{
    ...
    if ( _date == Nil )
    {
        [Logger log:self method:@"run" format:@"'date' property is not set"];
        [self complete:NO];
        return;
    }
    ...
    NSString* formattedDate = [NSString stringWithFormat:@"%04ld%02ld%02ld", (long)[components year], (long)[components month],(long)[components day]];
    NSString* uuid = [[[[UIDevice currentDevice] identifierForVendor] UUIDString] substringToIndex:12];
    nwReq.URL = [NSMutableString stringWithFormat:@"%@/open/%@/?day=%@",url,uuid, formattedDate];
...
- (void) complete: (BOOL) isSuccessfull
{
    ...
    if ( _delegate && [_delegate respondsToSelector:@selector(openShiftComplete:)])
        [_delegate openShiftComplete:_resultCode];
}
